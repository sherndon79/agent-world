#!/usr/bin/env python3
"""
WorldViewer Configuration

Handles environment variables and configuration for the WorldViewer MCP server.
"""
...
try:
    from agentworld_core.config import create_worldviewer_config
except ImportError:  # pragma: no cover - attempt to locate source checkout
    if _ensure_core_path():
        try:
            from agentworld_core.config import create_worldviewer_config
        except ImportError:  # pragma: no cover
            create_worldviewer_config = None
    else:
        create_worldviewer_config = None

_unified_config = create_worldviewer_config() if create_worldviewer_config else None


class WorldViewerConfig:
    """Configuration manager for WorldViewer MCP server."""
    ...
    @property
    def worldviewer_base_url(self) -> str:
        """Get the WorldViewer extension base URL."""
        return (
