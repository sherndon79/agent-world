"""
WorldStreamer NDI Extension

Isaac Sim NDI streaming control extension.
Provides API for starting, stopping, and managing NDI streaming sessions.
"""
...
class WorldStreamerExtension(omni.ext.IExt):
    """
    WorldStreamer Extension for Isaac Sim NDI streaming control.
    ...
    def on_startup(self, ext_id: str):
        ...
        try:
            ...
            ndi_name = self._config.get('ndi_name', 'Isaac Sim - Agent World')
            try:
                enc_cfg = self._config.get_encoder_config()
                fps = int(enc_cfg.get('encoding_fps', 30))
            except Exception:
                fps = 30
            self._streaming = StreamingInterface(ndi_name=ndi_name, fps=fps)
    ...
    def _log_startup_summary(self):
        ...
        try:
            summary_lines = [
                ...
                f"NDI Name: {self._config.get('ndi_name', 'Isaac Sim - Agent World')}",
            ...
            try:
                ...
                if urls.get('success'):
                    ndi_name = urls['urls'].get('ndi_name', 'not available')
                    summary_lines.append(f"NDI Name: {ndi_name}")
            except Exception:
                summary_lines.append("NDI Name: not available")
    ...
    def _create_window(self):
        ...
        try:
            self._window = ui.Window("Agent WorldStreamer (NDI)", width=320, height=300)
            
            with self._window.frame:
                with ui.VStack(spacing=2):
                    # Title section
                    with ui.HStack(height=0):
                        ui.Label("Agent WorldStreamer (NDI) Extension", style={"font_size": 14})
                    ...
                    with ui.CollapsableFrame("Status", collapsed=False, height=0):
                        with ui.VStack(spacing=1):
                            ...
                            ui.Spacer(height=2)
                            ui.Label("NDI Streaming:", style={"font_size": 12})
                    ...
                    if streaming_status.get('success'):
                        ...
                        try:
                            ...
                            if urls_result.get('success'):
                                ...
                                ndi_name = urls.get('ndi_name', 'Not available')
                                ndi_info = f"NDI: {ndi_name}"
                                self._streaming_urls_label.text = ndi_info
                            else:
                                self._streaming_urls_label.text = "NDI: Not available"
                        except Exception:
                            self._streaming_urls_label.text = "NDI: Error"
                    else:
