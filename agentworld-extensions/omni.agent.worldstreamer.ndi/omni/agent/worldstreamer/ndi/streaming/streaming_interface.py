"""
Streaming Interface for WorldStreamer (NDI)

Core NDI streaming control interface for Isaac Sim viewport capture.
Provides continuous viewport streaming via GStreamer NDI plugin.
"""
...
class StreamingInterface:
    """
    Core NDI streaming interface for Isaac Sim viewport capture.

    Provides unified streaming control with viewport capture
    and GStreamer NDI pipeline.
    """

    def __init__(self, ndi_name: str = "Isaac Sim - Agent World", fps: int = 30):
        """
        Initialize NDI streaming interface.
        ...
        self._fps = int(fps) if fps and fps > 0 else 30  # Default 30fps
        ...
        logger.info(f"NDI StreamingInterface initialized - name: {self._ndi_name}, fps: {self._fps}")
    ...
    def start_streaming(self, server_ip: Optional[str] = None) -> Dict[str, Any]:
        """
        Start NDI streaming of Isaac Sim viewport.
        ...
        try:
            ...
            if url_result['success']:
                ...
                self._status_tracker.update_stream_info(0, url_result['urls'])
                ...
                logger.info("NDI streaming started successfully")

                return {
                    'success': True,
                    'message': 'NDI streaming started successfully',
                    'streaming_info': {
                        'ndi_name': self._ndi_name,
                        'fps': self._fps,
    ...
    def stop_streaming(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info("NDI streaming stopped successfully")
    ...
    def get_streaming_urls(self, server_ip: Optional[str] = None) -> Dict[str, Any]:
        """
        Get current NDI streaming information.

        Args:
            server_ip: Ignored (for backward compatibility)

        Returns:
            Dict with NDI information
        """
        try:
            return {
                'success': True,
            ...
            logger.error(f"Failed to get NDI info: {e}")
            return {
                'success': False,
                'error': f"NDI info generation failed: {e}"
            }
    ...
    def _initialize_encoder(self) -> Dict[str, Any]:
        """
        Initialize NDI streaming pipeline.
        ...
        try:
            ...
            try:
                ...
                config = get_config()
                ndi_name = config.get('ndi_name', self._ndi_name)
            except Exception:
                ndi_name = self._ndi_name
            ...
            if not self._encoder:
                return {
                    'success': False,
                    'error': 'NDI encoder not available (check GStreamer and NDI plugin)'
                }
            ...
            logger.info(f"NDI encoder initialized: {self._encoder.get_info()}")
            ...
            error_msg = f"NDI encoder initialization failed: {e}"
            logger.error(error_msg)
            return {
                'success': False,
                'error': error_msg
    ...
    def _streaming_loop(self):
        """
        Main streaming loop - captures viewport and feeds to NDI encoder.
        """
        logger.info("Starting NDI viewport capture streaming loop")
