"""
External streaming pipeline for WorldStreamer (NDI).

Provides NDI streaming via GStreamer NDI plugin.
NDI handles compression internally, so no separate encoding needed.

Designed for low-latency NDI streaming from Isaac Sim viewport capture.
...
class NDIEncoder(BaseEncoder):
    """GStreamer NDI encoder - no separate encoding needed, NDI handles compression."""

    def __init__(self, resolution: Tuple[int, int], fps: int, ndi_name: str):
        # NDI doesn't use rtmp_url or bitrate, pass dummy values to parent
        super().__init__(resolution, fps, ndi_name, bitrate_kbps=0)
    ...
    def initialize(self) -> Dict[str, Any]:
        """Initialize GStreamer NDI pipeline."""
        try:
            ...
            # Check NDI plugin availability
            if not validate_gstreamer_element_availability('ndisink'):
                return {
                    'success': False,
                    'error': 'GStreamer NDI plugin (ndisink) not available'
                }

            # Build GStreamer NDI pipeline using secure builder
            try:
                pipeline = create_secure_ndi_pipeline(
                ...
                logger.info(f"Starting GStreamer NDI pipeline: {' '.join(pipeline)}")
                ...
                logger.info(f"GStreamer NDI encoder initialized: {self.ndi_name}")
                return {
                    'success': True,
                    'encoder_type': 'ndi',
                    'ndi_name': self.ndi_name,
            ...
            logger.error(f"GStreamer NDI initialization failed: {e}")
            return {
                'success': False,
    ...
    def encode_frame(self, frame_data: bytes) -> bool:
        ...
        try:
            ...
            try:
                ...
                logger.debug("NDI queue full, dropping frame")
                return False
    ...
    def stop(self):
        ...
        try:
            ...
            logger.info("GStreamer NDI encoder stopped")

        except Exception as e:
            logger.error(f"Error stopping GStreamer NDI encoder: {e}")

    def get_info(self) -> str:
        """Get encoder information."""
        return f"GStreamer-NDI ({self.resolution[0]}x{self.resolution[1]}@{self.fps}fps, name={self.ndi_name})"
    ...
    def _encoder_loop(self):
        """Encoder thread loop - sends frames to GStreamer NDI."""
        logger.info("GStreamer NDI encoder loop started")
        ...
        try:
            while not self._stop_event.is_set() and self._process:
                try:
                    ...
                    if self._process and self._process.stdin and not self._process.stdin.closed:
                        try:
                            ...
                            if "closed file" in str(e):
                                logger.debug("NDI stdin closed during write, stopping encoder loop")
                                break
                            else:
                                raise
                    ...
                    logger.error("Broken pipe: NDI sink disconnected or pipeline failed")
                    self._active = False
                    break
                except Exception as e:
            ...
            logger.error(f"NDI encoder loop error: {e}")
        finally:
            logger.info("GStreamer NDI encoder loop ended")


class GStreamerEncoder(BaseEncoder):
    """GStreamer-SRT-based encoder with hardware acceleration support."""
    ...
    def get_info(self) -> str:
        """Get encoder information."""
        return f"GStreamer-SRT-{self.encoder_type} ({self.resolution[0]}x{self.resolution[1]}@{self.fps}fps)"
        ...
        try:
            while not self._stop_event.is_set() and self._process:
                try:
                    ...
                    logger.error("Broken pipe: SRT sink disconnected or pipeline failed")
...
def get_best_encoder(resolution: Tuple[int, int], fps: int, ndi_name: str, **kwargs) -> Optional[BaseEncoder]:
    """
    Get the best available encoder for NDI streaming.

    For NDI, we only use the NDI encoder as it handles everything internally.
    ...
    Returns:
        NDI encoder instance, or None if not available
    """
    try:
        # Check if GStreamer is available
        try:
        ...
        if not validate_gstreamer_element_availability('ndisink'):
            logger.error("GStreamer NDI plugin (ndisink) not available")
            return None

        # Create NDI encoder
        encoder = NDIEncoder(resolution, fps, ndi_name)
        logger.info(f"Selected NDI encoder (name={ndi_name})")
        return encoder

    except Exception as e:
        logger.error(f"NDI encoder selection failed: {e}")
        return None
