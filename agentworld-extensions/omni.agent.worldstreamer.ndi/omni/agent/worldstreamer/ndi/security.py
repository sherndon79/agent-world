"""
WorldStreamer Security Module

Authentication and security for WorldStreamer streaming control API.
...
class WorldStreamerAuth:
    """
    Authentication handler for WorldStreamer API.
    
    Provides consistent authentication using the unified agent_world system.
    Focused on streaming control API security only.
    """
    ...
    def __init__(self, config=None):
        ...
        if AUTH_AVAILABLE:
            try:
                self._security_manager = SecurityManager('worldstreamer', config=config)
                self._auth_enabled = self._security_manager.is_auth_enabled()
                logger.info("WorldStreamer authentication initialized with unified SecurityManager")
            except Exception as e:
                logger.error(f"Failed to initialize WorldStreamer authentication: {e}")
                self._auth_enabled = False
        else:
            logger.warning("WorldStreamer authentication unavailable - unified auth system not found")
            self._auth_enabled = False
    ...
    def get_auth_requirements(self) -> Dict[str, Any]:
        ...
        bearer_enabled = AUTH_AVAILABLE and is_bearer_auth_enabled('worldstreamer.ndi')
    ...
    def create_auth_error_response(self, error_message: str) -> Dict[str, Any]:
        """
    ...
    def get_health_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'bearer_auth_enabled': AUTH_AVAILABLE and is_bearer_auth_enabled('worldstreamer.ndi'),
            'extension_name': 'worldstreamer.ndi'
        }
