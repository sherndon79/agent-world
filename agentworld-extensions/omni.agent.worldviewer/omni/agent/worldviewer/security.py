"""
WorldViewer Security Module

Authentication and security for WorldViewer API.
...
class WorldViewerAuth:
    """
    Authentication handler for WorldViewer API.
    ...
    def __init__(self, config=None):
        ...
        if AUTH_AVAILABLE:
            try:
                self._security_manager = SecurityManager('worldviewer', config=config)
                logger.info("WorldViewer authentication initialized with unified SecurityManager")
            except Exception as e:
                logger.error(f"Failed to initialize WorldViewer authentication: {e}")
        else:
            logger.warning("WorldViewer authentication unavailable - unified auth system not found")
    ...
    def get_auth_requirements(self) -> Dict[str, Any]:
        ...
        bearer_enabled = AUTH_AVAILABLE and is_bearer_auth_enabled('worldviewer')
