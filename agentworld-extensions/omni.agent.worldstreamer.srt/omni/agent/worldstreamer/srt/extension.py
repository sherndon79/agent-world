"""
WorldStreamer SRT Extension

Isaac Sim SRT streaming control extension.
Provides API for starting, stopping, and managing SRT streaming sessions.
"""
...
class WorldStreamerExtension(omni.ext.IExt):
    """
    WorldStreamer Extension for Isaac Sim SRT streaming control.
    ...
    def on_startup(self, ext_id: str):
        ...
        try:
            ...
            # Reload unified config to pick up recent changes
            ...
            rtmp_port = self._config.get('rtmp_port', 1935)  # Unused for SRT; kept for API parity
            stream_key = "live"  # Unused for SRT
            try:
                enc_cfg = self._config.get_encoder_config()
                fps = int(enc_cfg.get('encoding_fps', 24))
            except Exception:
                fps = 24
            # Pass SRT URI to streaming interface to avoid stale config reads
            srt_uri = self._config.get('srt_url', None)
            self._streaming = StreamingInterface(rtmp_port=rtmp_port, stream_key=stream_key, fps=fps, srt_uri=srt_uri)
    ...
    def _log_startup_summary(self):
        ...
        try:
            summary_lines = [
                ...
                f"SRT URI: {self._config.get('srt_url', 'unset')}",
            ...
            try:
                ...
                if urls.get('success'):
                    srt_uri = urls['urls'].get('srt_uri', 'not available')
                    summary_lines.append(f"SRT URI: {srt_uri}")
            except Exception:
                summary_lines.append("SRT URI: not available")
    ...
    def _create_window(self):
        ...
        try:
            self._window = ui.Window("Agent WorldStreamer (SRT)", width=320, height=300)
            
            with self._window.frame:
                with ui.VStack(spacing=2):
                    # Title section
                    with ui.HStack(height=0):
                        ui.Label("Agent WorldStreamer (SRT) Extension", style={"font_size": 14})
                    ...
                    with ui.CollapsableFrame("Status", collapsed=False, height=0):
                        with ui.VStack(spacing=1):
                            ...
                            ui.Spacer(height=2)
                            ui.Label("SRT Streaming:", style={"font_size": 12})
                    ...
                    if streaming_status.get('success'):
                        ...
                        try:
                            ...
                            if urls_result.get('success'):
                                ...
                                srt_uri = urls.get('srt_uri', 'Not available')
                                self._streaming_urls_label.text = f"SRT: {srt_uri[:64]}..."
                            else:
