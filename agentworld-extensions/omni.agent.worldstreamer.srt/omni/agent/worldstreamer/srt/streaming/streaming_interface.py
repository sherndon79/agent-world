"""
Streaming Interface for WorldStreamer (SRT)

Core SRT streaming control interface for Isaac Sim viewport capture.
Provides 24fps continuous viewport streaming with external encoding pipeline (GStreamer/NVENC/fallbacks).
"""
...
class StreamingInterface:
    """
    Core SRT streaming interface for Isaac Sim viewport capture.
    
    Provides unified streaming control with 24fps viewport capture
    and external encoding pipeline (GStreamer/NVENC/fallbacks).
    """
    
    def __init__(self, rtmp_port: int = 1935, stream_key: str = "live", fps: int = 24, srt_uri: Optional[str] = None):
        """
        ...
        # Preferred SRT URI override (from extension/config)
        self._srt_uri_override = srt_uri
        logger.info(f"SRT StreamingInterface initialized - fps: {self._fps}")
    ...
    def start_streaming(self, server_ip: Optional[str] = None) -> Dict[str, Any]:
        """
        Start SRT streaming of Isaac Sim viewport.
        ...
        try:
            ...
            # Provide SRT URI info
            url_result = {
                'success': True,
                'urls': {
                    'srt_uri': self._build_srt_uri()
                }
            }
            ...
            if url_result['success']:
                ...
                logger.info("SRT streaming started successfully")
                
                return {
                    'success': True,
                    'message': 'Streaming started successfully',
                    'streaming_info': {
                        'srt_uri': url_result['urls']['srt_uri'],
                        'fps': self._fps,
    ...
    def stop_streaming(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info("SRT streaming stopped successfully")
    ...
    def _build_srt_uri(self) -> str:
        try:
            # Use override if provided by extension on startup
            if getattr(self, '_srt_uri_override', None):
                return self._ensure_streamid(self._srt_uri_override)  # type: ignore[attr-defined]
            from ..config import get_config
            url = get_config().get('srt_url')
            if url:
    ...
    def _ensure_streamid(self, url: str) -> str:
        ...
        This makes the stream visible at app=app, stream=isaac in OME.
        """
        try:
            ...
            sid = q.get('streamid')
            wanted = 'default/app/isaac'
            def _ok(v: str) -> bool:
                parts = (v or '').split('/')
                return len(parts) >= 3 and all(parts[:3])
            if not sid or not _ok(sid):
                q['streamid'] = wanted
            ...
            if 'streamid=' not in url:
                sep = '&' if ('?' in url) else '?'
                return f"{url}{sep}streamid=default/app/isaac"
            return url
