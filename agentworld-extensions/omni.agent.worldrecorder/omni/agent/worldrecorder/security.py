"""
Security and rate limiting for WorldRecorder API (unified pattern).
"""
import hashlib
import hmac
import os
...
class SecurityManager:
    def __init__(self, settings_path: str = "/exts/omni.agent.worldrecorder/auth_enabled",
                 bearer_env: str = 'AGENT_WORLDRECORDER_AUTH_TOKEN',
                 hmac_env: str = 'AGENT_WORLDRECORDER_HMAC_SECRET'):
    ...
    def check_auth(self, method: str, headers: Dict, path: str) -> bool:
        try:
            if (os.getenv('AGENT_EXT_AUTH_ENABLED') or '').lower() in ('0','false','no','off'):
                return True
            # Honor extension settings toggle if available
            try:
                import carb
