"""
WorldRecorder Security Module

Authentication and security for WorldRecorder API.
...
def _ensure_core_path() -> bool:
    """Ensure agentworld_core is importable when running from a source checkout."""
...
# Ensure the shared core helpers are importable before attempting to use them.
_ensure_core_path()
...
try:
    ...
    if _ensure_core_path():
        try:
            from agentworld_core.auth import SecurityManager, is_bearer_auth_enabled
            AUTH_AVAILABLE = True
        except ImportError as exc:  # pragma: no cover - only when core truly missing
...
class WorldRecorderAuth:
    """
    Authentication handler for WorldRecorder API.
    ...
    def __init__(self, config=None):
        ...
        if AUTH_AVAILABLE:
            try:
                self._security_manager = SecurityManager('worldrecorder', config=config)
                logger.info("WorldRecorder authentication initialized with unified SecurityManager")
            except Exception as e:
                logger.error(f"Failed to initialize WorldRecorder authentication: {e}")
        else:
            logger.warning("WorldRecorder authentication unavailable - unified auth system not found")
    ...
    def get_auth_requirements(self) -> Dict[str, Any]:
        ...
        bearer_enabled = AUTH_AVAILABLE and is_bearer_auth_enabled('worldrecorder')
    ...
    def get_health_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'bearer_auth_enabled': AUTH_AVAILABLE and is_bearer_auth_enabled('worldrecorder'),
            'extension_name': 'worldrecorder'
        }
