"""
Connection Manager for WorldStreamer

Handles network connections, IP detection, and RTMP streaming URL management.
Focused solely on RTMP streaming connection infrastructure.
"""
...
class ConnectionManager:
    """
    Manages network connections and IP detection for RTMP streaming.
    
    Handles public IP detection, connection validation, and RTMP 
    connection URL generation for streaming clients and servers.
    """
    ...
    def generate_streaming_urls(self, rtmp_port: int, server_ip: Optional[str] = None, 
                              stream_key: str = "live") -> Dict[str, Any]:
        """
        Generate RTMP streaming URLs for client connection and server configuration.
        
        Args:
            rtmp_port: RTMP streaming port (usually 1935)
            server_ip: Optional server IP override
            stream_key: RTMP stream key/path
            
        Returns:
            Dict with streaming URLs and connection information
        """
        try:
            urls = {
                'rtmp_base_url': f"rtmp://localhost:{rtmp_port}",
                'rtmp_stream_url': f"rtmp://localhost:{rtmp_port}/{stream_key}",
                'connection_info': {
                    'port': rtmp_port,
                    'protocol': 'RTMP',
            ...
            # Add local network URLs
            local_ip = self.get_local_ip()
            if local_ip:
                urls['local_network_rtmp_url'] = f"rtmp://{local_ip}:{rtmp_port}/{stream_key}"
                urls['local_network_base_url'] = f"rtmp://{local_ip}:{rtmp_port}"
                urls['connection_info']['local_ip'] = local_ip
            
            # Add public URLs if available
            public_ip = server_ip or self.get_public_ip()
            if public_ip:
                urls['public_rtmp_url'] = f"rtmp://{public_ip}:{rtmp_port}/{stream_key}"
                urls['public_base_url'] = f"rtmp://{public_ip}:{rtmp_port}"
                urls['connection_info']['public_ip'] = public_ip
            
            # Add client consumption URLs (for Agent Adventures platform)
            urls['client_urls'] = self._generate_client_urls(rtmp_port, local_ip, public_ip, stream_key)
            
            # Add streaming platform URLs (Twitch, YouTube, etc.)
            urls['platform_examples'] = self._generate_platform_examples(stream_key)
            
            # Add connection recommendations
            urls['recommendations'] = self._get_rtmp_recommendations()
            ...
            logger.error(f"RTMP URL generation failed: {e}")
            return {
                'success': False,
                'error': str(e)
            }
