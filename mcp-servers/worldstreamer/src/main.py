#!/usr/bin/env python3
"""
Isaac Sim WorldStreamer MCP Server (Stdio)

Provides Claude Code with direct Isaac Sim streaming capabilities through MCP tools.
Interfaces with the Agent WorldStreamer Extension HTTP API.
...
async def main():
    """Main entry point for the WorldStreamer MCP server."""
    # Setup logging for stdio MCP - disable console to avoid stdout contamination
    os.environ.setdefault('AGENT_LOG_FILE', '/tmp/worldstreamer_mcp.log')
    ...
    logger.info("🚀 Starting Isaac Sim WorldStreamer MCP Server (Stdio - Modular)")
    ...
    logger.info("WorldStreamer MCP Server ready for stdio communication")
    ...
    try:
        ...
        # Cleanup
        await close_client()
        logger.info("WorldStreamer MCP Server shutdown complete")
