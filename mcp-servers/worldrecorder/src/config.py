#!/usr/bin/env python3
"""
WorldRecorder Configuration

Handles environment variables and configuration for the WorldRecorder MCP server.
"""
...
try:
    from agentworld_core.config import create_worldrecorder_config
except ImportError:  # pragma: no cover - attempt to locate source checkout
    if _ensure_core_path():
        try:
            from agentworld_core.config import create_worldrecorder_config
        except ImportError:  # pragma: no cover
            create_worldrecorder_config = None
    else:
        create_worldrecorder_config = None

_unified_config = create_worldrecorder_config() if create_worldrecorder_config else None


class WorldRecorderConfig:
    """Configuration manager for WorldRecorder MCP server."""
    ...
    @property
    def worldrecorder_base_url(self) -> str:
        """Get the WorldRecorder extension base URL."""
        return (
