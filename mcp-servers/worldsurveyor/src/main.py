#!/usr/bin/env python3
"""
Isaac Sim Worldsurveyor MCP Server (Stdio)

Provides Claude Code with direct Isaac Sim worldsurveyor capabilities through MCP tools.
Interfaces with the Agent Worldsurveyor Extension HTTP API.
...
async def main():
    """Main entry point for the Worldsurveyor MCP server."""
    # Setup logging for stdio MCP - disable console to avoid stdout contamination
    os.environ.setdefault('AGENT_LOG_FILE', '/tmp/worldsurveyor_mcp.log')
    ...
    logger.info("🚀 Starting Isaac Sim Worldsurveyor MCP Server (Stdio - Modular)")
    ...
    logger.info("Worldsurveyor MCP Server ready for stdio communication")
    ...
    try:
        ...
        # Cleanup
        await close_client()
        logger.info("Worldsurveyor MCP Server shutdown complete")
